...
export default function DeleteDataButton() {
  ...
  return (
    <div className="py-2">
      ...
      {message && (
        <div className={`mt-3 text-sm font-medium p-3 ${message.includes('successfully') ? 'bg-accent-surface text-accent' : 'bg-error-surface text-error'}`} style={{ borderRadius: 'var(--radius-sm)' }}>
          {message}
        </div>
      )}
